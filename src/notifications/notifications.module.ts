...
export class NotificationsModule implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (serviceAccountPath && fs.existsSync(serviceAccountPath)) {
      ...
      try {
        admin.initializeApp();
        this.logger.warn('Firebase Admin initialized without service account — FCM pushes will not work');
      } catch {
        this.logger.warn('Firebase Admin could not be initialized — FCM pushes will not work');
      }
    }
  }
}

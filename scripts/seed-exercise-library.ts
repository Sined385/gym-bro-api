...
async function main() {
  ...
  try {
    ...
    // Ukrainian display names keyed by external_id — generated by
    // scripts/generate-exercise-name-translations.ts. Tolerate a
    // missing file so the seed still works on a fresh checkout before
    // translations were generated (name_uk just stays NULL).
    const translationsPath = path.join(
      __dirname,
      '..',
      'data',
      'exercise-names.uk.json',
    );
    ...
    const mapped = filtered.map((e) => {
      ...
      return {
        name: e.name,
        name_uk: translations[e.id] ?? null,
